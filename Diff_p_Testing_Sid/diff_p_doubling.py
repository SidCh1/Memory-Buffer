...
def create_elementary_links(memory_numbers=np.full(8, 1),
                            generation_probability=[1]):
    """Creates a vector of elementary links with a distribution of memory_buffers and unique probabilities per segment."""

    # Validate memory_numbers length for doubling scheme
    ...
    num_segments = len(memory_numbers) // 2
    
    # Validate the generation_probability length
    if len(generation_probability) != num_segments:
        raise ValueError("The length of generation_probability must match the number of segments.")

    linklist = []

    # Create links with respective probabilities for each segment
    for i in range(num_segments):
        memoryL = Memory(buffer_space=memory_numbers[2 * i])
        memoryR = Memory(buffer_space=memory_numbers[2 * i + 1])
        
        # Assign the generation probability for this segment
        link = Elementary_Link(memoryL=memoryL, memoryR=memoryR, generation_probability=generation_probability[i])
...
def create_chain_doubling(memory_numbers=np.full(8, 1),
                          generation_probability=[1],
                          swapping_probability=1):    
    """Creates a list of lists, where the first list element is a list of elementary links (step 1), 
    the second list element is a list of longer links (step 2), and so on."""

    links = []
    
    # Create initial elementary links with segment-specific generation probabilities
    ...
    # Doubling chain creation, creating longer links in each step
...
def get_statistics(memory_numbers=np.full(8, 1),
    ...
    """Calculates the mean and error of waiting times based on the distribution of memory buffers, 
    generation probabilities for each segment, and swapping probability."""

    waitingtimes = []
    chains = []

    # Repeat the chain creation process to gather statistics
    for _ in range(number_of_repetitions):
        # Pass list of generation probabilities to create_chain_doubling
    ...
    # Calculate waiting time for each chain and gather statistics
    for chain in chains: 
...
if __name__ == "__main__":
    ...
    generation_probability = [0.01, 0.02, 0.03, 0.04]  # List of probabilities for each segment
    ...
    print("simulation parameters: \n generation probabilities = ", generation_probability, 
    ...
    # Calculate the average generation probability
    avg_generation_probability = sum(generation_probability) / len(generation_probability)
    
    q = 1. - avg_generation_probability  # Use the average probability for the analytic calculation

    # q = 1. - generation_probability[0] # Just giving the first segment gen probability here
