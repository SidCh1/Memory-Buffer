...
memory_numbers_list = [[4,4,4,4,4,4,4,4],[3,3,3,3,7,7,3,3]]
...
file_name_list = ["compare_fixed_length_32-4.csv", "compare_fixed_length_32-un.csv"] ### file names: compare_fixed_buffers_const-bfix_.csv
...
for m in range(2):
    ...
    memory_numbers = memory_numbers_list[m]# memory distribution list
    element_numbers = 4 # no of segments
    file_name = file_name_list[m-1] # file name to write into csv
    ...
    for sw_prob in reversed(dataa):
        ...
        with open(file_name, mode='a') as csvfile:
            ...
            csvfile_writer.writerow([element_numbers, gen_prob, sw_prob, data[0], data[1], number_of_repetitions, a, data[1]/data[0]]) #data[0] is mean and [1] is error of AWT
